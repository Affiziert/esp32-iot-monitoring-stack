...
The project uses battery-powered ESP32-C3 sensor nodes to acquire temperature, relative humidity and battery voltage. The sensor data is transmitted wirelessly via ESP-NOW to an ESP32-C3 receiver and forwarded via USB serial to a Raspberry Pi. A Python-based data bridge processes the received data and stores it in an InfluxDB database.
...
- ESP32-C3 transmitter node
- ESP32-C3 receiver node
- ESP-NOW wireless communication
- Raspberry Pi 4 as edge gateway
- Python data bridge script
...
- End-to-end IoT monitoring stack
- Battery-powered ESP32-C3 sensor nodes
- ESP-NOW based wireless communication
- Temperature measurement
- Relative humidity measurement
- Battery voltage monitoring
- Low-power operation using deep sleep
- Raspberry Pi edge processing
- JSON serial data interface
- Python data bridge
- Docker-compose InfluxDB deployment
...
The transmitter periodically wakes from deep sleep, measures the connected sensors, transmits the data and returns to deep sleep.

### ESP32 Receiver Node

The receiver node continuously listens for ESP-NOW packets and forwards valid sensor data via USB serial.

The received data is transmitted as a JSON object containing:
...
5. Automatically reconnect if the serial connection is interrupted.
...
The actual InfluxDB token should not be committed to the repository.
...
The firmware version identifies the software version installed on an ESP32 device.

The firmware version is independent of the communication protocol version and can be used to identify the exact firmware running on a device during debugging.
...
- [x] ESP32-C3 transmitter firmware
- [x] ESP32-C3 receiver firmware
- [x] ESP-NOW communication
- [x] Protocol version handling
- [x] Firmware version identification
- [x] SHT31 temperature and humidity measurement
- [x] Battery voltage measurement
- [x] Deep sleep operation
- [x] JSON serial output
- [x] Python data bridge
- [x] InfluxDB integration
- [x] Serial reconnect handling
- [x] Debug output
